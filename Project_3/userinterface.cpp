...
void UserInterface::initialQuestion()
{
    ...
    while (true)
    {
        ...
        {
            std::cout << "Please enter a directory filepath (press Enter to use default 'data/archive')" << std::endl;
            std::string answer2;
            std::getline(std::cin, answer2);
            
            // Use default path if empty input
            if (answer2.empty()) {
                answer2 = "data/archive";
            }
            
            auto startTrain = std::chrono::high_resolution_clock::now();
            std::cout << "Reading files from " << answer2 << "..." << std::endl;
